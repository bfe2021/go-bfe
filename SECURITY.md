# Security Policy

## Supported Versions

Please see [Releases](https://github.com/bfe2021/go-bfe/releases). We recommend using the [most recently released version](https://github.com/bfe2021/go-bfe/releases/latest).

## Audit reports

Audit reports are published in the `docs` folder: https://github.com/bfe2021/go-bfe/tree/master/docs/audits 

| Scope | Date | Report Link |
| ------- | ------- | ----------- |
| `gbfe` | 20170425 | [pdf](https://github.com/bfe2021/go-bfe/blob/master/docs/audits/2017-04-25_Gbfe-audit_Truesec.pdf) |
| `clef` | 20180914 | [pdf](https://github.com/bfe2021/go-bfe/blob/master/docs/audits/2018-09-14_Clef-audit_NCC.pdf) |

## Reporting a Vulnerability

**Please do not file a public ticket** mentioning the vulnerability.

To find out how to disclose a vulnerability in Bfedu visit [https://bounty.bfedu.io](https://bounty.bfedu.io) or email bounty@bfedu.io. Please read the [disclosure page](https://github.com/bfe2021/go-bfe/security/advisories?state=published) for more information about publically disclosed security vulnerabilities.

Use the built-in `gbfe version-check` feature to check whether the software is affected by any known vulnerability. This command will fetch the latest [`vulnerabilities.json`](https://gbfe.bfedu.io/docs/vulnerabilities/vulnerabilities.json) file which contains known security vulnerabilities concerning `gbfe`, and cross-check the data against its own version number.

The following key may be used to communicate sensitive information to developers.

Fingerprint: `AE96 ED96 9E47 9B00 84F3 E17F E88D 3334 FA5F 6A0A`

```
-----BEGIN PGP PUBLIC KEY BLOCK-----
Version: GnuPG v1


=r6KK
-----END PGP PUBLIC KEY BLOCK-----
```
