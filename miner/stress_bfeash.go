...
// This file contains a miner stress test based on the Bfeash consensus engine.
package main
...
func main() {
	...
	// Pre-generate the bfeash mining DAG so we don't race
	bfeash.MakeDataset(1, filepath.Join(os.Getenv("HOME"), ".bfeash"))

	// Create an Bfeash network based off of the Ropsten config
...
// makeGenesis creates a custom Bfeash genesis block based on some pre-defined
// faucet accounts.
...
func makeMiner(genesis *core.Genesis) (*node.Node, *bfe.Bfedu, error) {
	// Define the basic configurations for the Bfedu node
	...
	// Create the node and configure a full Bfedu node on it
	...
	if err != nil {
	 ...
	 bfeBackend, err := bfe.New(stack, &bfeconfig.Config{
		...
		GPO:             bfe.DefaultConfig.GPO,
		Bfeash:          bfe.DefaultConfig.Bfeash,
		Miner: miner.Config{
