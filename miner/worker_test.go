...
var (
	...
	bfeashChainConfig *params.ChainConfig
...
func init() {
	...
	bfeashChainConfig = params.TestChainConfig
...
func newTestWorkerBackend(t *testing.T, chainConfig *params.ChainConfig, engine consensus.Engine, db bfedb.Database, n int) *testWorkerBackend {
...
func newTestWorker(t *testing.T, chainConfig *params.ChainConfig, engine consensus.Engine, db bfedb.Database, blocks int) (*worker, *testWorkerBackend) {
	backend := newTestWorkerBackend(t, chainConfig, engine, db, blocks)
	backend.txPool.AddLocals(pendingTxs)
	w := newWorker(testConfig, chainConfig, engine, backend, new(event.TypeMux), nil, false)
	w.setBfeerbase(testBankAddress)
	return w, backend
}

func TestGenerateBlockAndImportBfeash(t *testing.T) {
...
func TestEmptyWorkBfeash(t *testing.T) {
	testEmptyWork(t, bfeashChainConfig, bfeash.NewFaker())
}
...
func TestStreamUncleBlock(t *testing.T) {
	bfeash := bfeash.NewFaker()
	defer bfeash.Close()

	w, b := newTestWorker(t, bfeashChainConfig, bfeash, rawdb.NewMemoryDatabase(), 1)
	defer w.close()
...
func TestRegenerateMiningBlockBfeash(t *testing.T) {
	testRegenerateMiningBlock(t, bfeashChainConfig, bfeash.NewFaker())
}
...
func TestAdjustIntervalBfeash(t *testing.T) {
	testAdjustInterval(t, bfeashChainConfig, bfeash.NewFaker())
}
