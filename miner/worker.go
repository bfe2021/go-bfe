...
func newWorker(config *Config, chainConfig *params.ChainConfig, engine consensus.Engine,  bfe  Backend, mux *event.TypeMux, isLocalBlock func(*types.Block) bool, init bool) *worker {
	worker := &worker{
		...
		unconfirmed:        newUnconfirmedBlocks(bfe.BlockChain(), miningLogAtDepth),
	...
	worker.txsSub = bfe.TxPool().SubscribeNewTxsEvent(worker.txsCh)
	// Subscribe events for blockchain
	worker.chainHeadSub = bfe.BlockChain().SubscribeChainHeadEvent(worker.chainHeadCh)
	worker.chainSideSub = bfe.BlockChain().SubscribeChainSideEvent(worker.chainSideCh)
...
func (w *worker) commitNewWork(interrupt *int32, noempty bool, timestamp int64) {
	...
	pending, err := w.bfe.TxPool().Pending()
	if err != nil {
	...
	for _, account := range w.bfe.TxPool().Locals() {
...
// totalFees computes total consumed fees in BFE. Block transactions and receipts have to have the same order.
func totalFees(block *types.Block, receipts []*types.Receipt) *big.Float {
	feesWei := new(big.Int)
	for i, tx := range block.Transactions() {
		feesWei.Add(feesWei, new(big.Int).Mul(new(big.Int).SetUint64(receipts[i].GasUsed), tx.GasPrice()))
	}
	return new(big.Float).Quo(new(big.Float).SetInt(feesWei), new(big.Float).SetInt(big.NewInt(params.Bfeer)))
}
