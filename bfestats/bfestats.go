...
// Package bfestats implements the network stats reporting service.
package bfestats
...
import (
	...
	bfeproto "github.com/bfe2021/go-bfe/bfe/protocols/bfe"
...
// backend encompasses the bare-minimum functionality needed for bfestats reporting
...
// Service implements an Bfedu netstats reporting daemon that pushes local
...
func New(node *node.Node, backend backend, engine consensus.Engine, url string) error {
	...
	node.RegisterLifecycle(bfestats)
	return nil
}
...
func (s *Service) login(conn *connWrapper) error {
	...
	var network string
	if info := infos.Protocols["bfe"]; info != nil {
		network = fmt.Sprintf("%d", info.(*bfeproto.NodeInfo).Network)
	} else {
...
func (s *Service) reportLatency(conn *connWrapper) error {
	// Send the current time to the bfestats server
	start := time.Now()
	...
	log.Trace("Sending measured latency to bfestats", "latency", latency)
...
func (s *Service) reportBlock(conn *connWrapper, block *types.Block) error {
	...
	log.Trace("Sending new block to bfestats", "number", details.Number, "hash", details.Hash)
...
func (s *Service) reportHistory(conn *connWrapper, list []uint64) error {
	...
	if len(history) > 0 {
		log.Trace("Sending historical blocks to bfestats", "first", history[0].Number, "last", history[len(history)-1].Number)
	} else {
...
func (s *Service) reportPending(conn *connWrapper) error {
	...
	log.Trace("Sending pending transactions to bfestats", "count", pending)
...
func (s *Service) reportStats(conn *connWrapper) error {
	...
	log.Trace("Sending node details to bfestats")
