...
import (
	...
	"github.com/bfe2021/go-bfe/accounts/usbwallet/trezor"
...
// or whether the Bfedu app was not started on it.
...
// Bfedu address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.BfeduAddress)
	if _, err := w.trezorExchange(&trezor.BfeduGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	response := new(trezor.BfeduTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.BfeduTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
