...
func (g *gbferpc) killAndWait() {
	g.gbfe.Kill()
	g.gbfe.WaitExit()
}

func (g *gbferpc) callRPC(result interface{}, Method string, args ...interface{}) {
	if err := g.rpc.Call(&result, Method, args...); err != nil {
		g.gbfe.Fatalf("callRPC %v: %v", Method, err)
	}
}

func (g *gbferpc) addPeer(peer *gbferpc) {
	g.gbfe.Logf("%v.addPeer(%v)", g.name, peer.name)
	enode := peer.getNodeInfo().Enode
	peerCh := make(chan *p2p.PeerEvent)
	sub, err := g.rpc.Subscribe(context.Background(), "admin", peerCh, "peerEvents")
	if err != nil {
		g.gbfe.Fatalf("subscribe %v: %v", g.name, err)
	}
	...
	select {
	case ev := <-peerCh:
		g.gbfe.Logf("%v received event: type=%v, peer=%v", g.name, ev.Type, ev.Peer)
	case err := <-sub.Err():
		g.gbfe.Fatalf("%v sub error: %v", g.name, err)
	case <-timeout:
		g.gbfe.Error("timeout adding peer after", dur)
	}
}

// Use this function instead of `g.nodeInfo` directly
func (g *gbferpc) getNodeInfo() *p2p.NodeInfo {
	if g.nodeInfo != nil {
		return g.nodeInfo
	}
...
func (g *gbferpc) waitSynced() {
	// Check if it's synced now
	var result interface{}
	g.callRPC(&result, "bfe_syncing")
	syncing, ok := result.(bool)
	if ok && !syncing {
		g.gbfe.Logf("%v already synced", g.name)
		return
	}
	...
	select {
	case ev := <-ch:
		g.gbfe.Log("'syncing' event", ev)
		syncing, ok := ev.(bool)
		if ok && !syncing {
			break
		}
		g.gbfe.Log("Other 'syncing' event", ev)
	case err := <-sub.Err():
		g.gbfe.Fatalf("%v notification: %v", g.name, err)
		break
	case <-timeout:
		g.gbfe.Fatalf("%v timeout syncing", g.name)
		break
	}
}
...
func startGbfeWithIpc(t *testing.T, name string, args ...string) *gbferpc {
	ipcName := fmt.Sprintf("gbfe-%d.ipc", atomic.AddUint32(&nextIPC, 1))
	...
	var err error
	ipcpath := ipcEndpoint(ipcName, g.gbfe.Datadir)
...
func initGbfe(t *testing.T) string {
	args := []string{"--networkid=42", "init", "./testdata/clique.json"}
	t.Logf("Initializing gbfe: %v ", args)
	g := runGbfe(t, args...)
	datadir := g.Datadir
	g.WaitExit()
	return datadir
...
func startLightServer(t *testing.T) *gbferpc {
	datadir := initGbfe(t)
	t.Logf("Importing keys to gbfe")
	runGbfe(t, "--datadir", datadir, "--password", "./testdata/password.txt", "account", "import", "./testdata/key.prv", "--lightkdf").WaitExit()
	account := "0x02f0d131f1f97aef08aec6e3291b957d9efe7105"
	server := startGbfeWithIpc(t, "lightserver", "--allow-insecure-unlock", "--datadir", datadir, "--password", "./testdata/password.txt", "--unlock", account, "--mine", "--light.serve=100", "--light.maxpeers=1", "--nodiscover", "--nat=extip:127.0.0.1", "--verbosity=4")
	return server
}

func startClient(t *testing.T, name string) *gbferpc {
	datadir := initGbfe(t)
	return startGbfeWithIpc(t, name, "--datadir", datadir, "--nodiscover", "--syncmode=light", "--nat=extip:127.0.0.1", "--verbosity=4")
}
