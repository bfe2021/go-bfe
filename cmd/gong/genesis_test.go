...
// Tests that initializing Gbfe with a custom genesis block and chain definitions
// work properly.
func TestCustomGenesis(t *testing.T) {
	for i, tt := range customGenesisTests {
		...
		runGbfe(t, "--datadir", datadir, "init", json).WaitExit()

		// Query the custom genesis block
		gbfe := runGbfe(t, "--networkid", "1337", "--syncmode=full",
			"--datadir", datadir, "--maxpeers", "0", "--port", "0",
			"--nodiscover", "--nat", "none", "--ipcdisable",
			"--exec", tt.query, "console")
		gbfe.ExpectRegexp(tt.result)
		gbfe.ExpectExit()
	}
}
