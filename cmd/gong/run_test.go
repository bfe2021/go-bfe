...
func tmpdir(t *testing.T) string {
	dir, err := ioutil.TempDir("", "gbfe-test")
	if err != nil {
		t.Fatal(err)
	}
	return dir
...
func init() {
	// Run the app if we've been exec'd as "gbfe-test" in runGbfe.
	reexec.Register("gbfe-test", func() {
...
// spawns gbfe with the given command line args. If the args don't set --datadir, the
// child g gets a temporary data directory.
func runGbfe(t *testing.T, args ...string) *testgbfe {
	tt := &testgbfe{}
	...
	for i, arg := range args {
		...
		case "--miner.bfeerbase":
			if i < len(args)-1 {
				tt.Bfeerbase = args[i+1]
			}
		}
	}
	if tt.Datadir == "" {
	...
	// Boot "gbfe". This actually runs the test binary but the TestMain
	// function will prevent any tests from running.
	tt.Run("gbfe-test", args...)
