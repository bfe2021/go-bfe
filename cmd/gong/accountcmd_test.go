...
func TestAccountListEmpty(t *testing.T) {
	gbfe := runGbfe(t, "account", "list")
	gbfe.ExpectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	gbfe := runGbfe(t, "account", "list", "--datadir", datadir)
	defer gbfe.ExpectExit()
...
func TestAccountNew(t *testing.T) {
	gbfe := runGbfe(t, "account", "new", "--lightkdf")
	defer gbfe.ExpectExit()
	gbfe.Expect(`
...
func importAccountWithExpect(t *testing.T, key string, expected string) {
	...
	gbfe := runGbfe(t, "account", "import", keyfile, "-password", passwordFile)
	defer gbfe.ExpectExit()
	gbfe.Expect(expected)
}
...
func TestWalletImport(t *testing.T) {
	gbfe := runGbfe(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer gbfe.ExpectExit()
	gbfe.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(gbfe.Datadir, "keystore"))
	if len(files) != 1 {
...
func TestUnlockFlag(t *testing.T) {
	gbfe := runMinimalGbfe(t, "--port", "0", "--ipcdisable", "--datadir", tmpDatadirWithKeystore(t),
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(gbfe.StderrText(), m) {
...
// https://github.com/bfe2021/go-bfe/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	gbfe.SetTemplateFunc("keypath", func(file string) string {
