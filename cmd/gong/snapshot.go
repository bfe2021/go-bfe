...
var (
	snapshotCommand = cli.Command{
		...
		Subcommands: []cli.Command{
			{
				Name:      "prune-state",
				Usage:     "Prune stale bfedu state data based on the snapshot",
				ArgsUsage: "<root>",
				...
				Description: `
gbfe snapshot prune-state <state-root>
...
during the use of Gbfe, please also specify it here for correct deletion. Otherwise
...
`,
			},
			{
				...
				Description: `
gbfe snapshot verify-state <state-root>
...
`,
			},
			{
				...
				Description: `
gbfe snapshot traverse-state <state-root>
...
`,
			},
			{
				...
				Description: `
gbfe snapshot traverse-rawstate <state-root>
...
func pruneState(ctx *cli.Context) error {
	...
	pruner, err := pruner.NewPruner(chaindb, chain.CurrentBlock().Header(), stack.ResolvePath(""), stack.ResolvePath(config.Bfe.TrieCleanCacheJournal), ctx.GlobalUint64(utils.BloomFilterSizeFlag.Name))
	if err != nil {
