...
// faucet is an Bfeer faucet backed by a light client.
package main
...
var (
	...
	payoutFlag  = flag.Int("faucet.amount", 1, "Number of Bfeers to pay out per user request")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for Bfedu and the faucet")
...
func main() {
	...
	for i := 0; i < *tiersFlag; i++ {
		...
		amounts[i] = fmt.Sprintf("%s Bfeers", strconv.FormatFloat(amount, 'f', -1, 64))
		if amount == 1 {
...
type request struct {
	...
	Account common.Address     `json:"account"` // Bfedu address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an Bfedu light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // Bfedu protocol stack
	client *bfeclient.Client   // Client connection to the Bfedu chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*enode.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the Bfedu light client protocol
	cfg := bfeconfig.Defaults
	...
	if err != nil {
		return nil, fmt.Errorf("Failed to register the Bfedu service: %w", err)
	}

	// Assemble the bfestats monitoring and reporting service'
	if stats != "" {
		if err := bfestats.New(stack, lesBackend.ApiBackend, lesBackend.Engine(), stats); err != nil {
			return nil, err
		}
	}
	...
	client := bfeclient.NewClient(api)
...
// close terminates the Bfedu connection and tears down the faucet.
...
// apiHandler handles requests for Bfeer grants and transaction statuses.
func (f *faucet) apiHandler(w http.ResponseWriter, r *http.Request) {
	...
	for {
		...
		// Retrieve the Bfedu address to fund, the requesting user and a profile picture
		var (
			id       string
			username string
			avatar   string
...
// the uniqueness identifier (user id/username), username, avatar URL and Bfedu address to fund on success.
func authTwitter(url string, tokenV1, tokenV2 string) (string, string, string, common.Address, error) {
	...
	// and scrape it for the Bfedu address and profile URL. We need to load
	...
	if address == (common.Address{}) {
		//lint:ignore ST1005 This error is to be displayed in the browser
		return "", "", "", common.Address{}, errors.New("No Bfedu address found to fund")
	}
	var avatar string
...
// API, returning the user id, username, avatar URL and Bfedu address to fund on
// success.
...
// returning the username, avatar URL and Bfedu address to fund on success.
func authFacebook(url string) (string, string, common.Address, error) {
	...
	// scrape it for the Bfedu address and profile URL.
	//
...
// authNoAuth tries to interpret a faucet request as a plain Bfedu address,
