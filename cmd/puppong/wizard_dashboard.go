...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"bfestats", "explorer", "wallet", "faucet"} {
		...
		for _, server := range available[service] {
			...
			switch service {
			case "bfestats":
				if infos, err := checkBfestats(client, w.network); err == nil {
					port = infos.port
				}
			case "explorer":
	...
	// If we have bfestats running, ask whether to make the secret public or not
	if w.conf.bfestats != "" {
		fmt.Println()
		fmt.Println("Include bfestats secret on dashboard (y/n)? (default = yes)")
