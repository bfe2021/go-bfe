...
FROM bfedu2020/client-go:alltools-latest
...
ENTRYPOINT [ \
	"faucet", "--genesis", "/genesis.json", "--network", "{{.NetworkID}}", "--bootnodes", "{{.Bootnodes}}", "--bfestats", "{{.Bfestats}}", "--ongport", "{{.BfePort}}",     \
...
services:
  faucet:
    ...
    ports:
      - "{{.BfePort}}:{{.BfePort}}"
      - "{{.BfePort}}:{{.BfePort}}/udp"{{if not .VHost}}
      - "{{.ApiPort}}:8080"{{end}}
    volumes:
      - {{.Datadir}}:/root/.faucet
    environment:
...
func deployFaucet(client *sshClient, network string, bootnodes []string, config *faucetInfos, nocache bool) ([]byte, error) {
	...
	template.Must(template.New("").Parse(faucetDockerfile)).Execute(dockerfile, map[string]interface{}{
		...
		"Bfestats":      config.node.bfestats,
		"BfePort":       config.node.port,
	...
	template.Must(template.New("").Parse(faucetComposefile)).Execute(composefile, map[string]interface{}{
		...
		"ApiPort":       config.port,
		"BfePort":       config.node.port,
		"BfeName":       config.node.bfestats[:strings.Index(config.node.bfestats, ":")],
		"CaptchaToken":  config.captchaToken,
		"CaptchaSecret": config.captchaSecret,
		"FaucetAmount":  config.amount,
		"FaucetMinutes": config.minutes,
...
func (info *faucetInfos) Report() map[string]string {
	report := map[string]string{
		...
		"Bfedu listener port":          strconv.Itoa(info.node.port),
		"Funding amount (base tier)":   fmt.Sprintf("%d Bfeers", info.amount),
		"Funding cooldown (base tier)": fmt.Sprintf("%d mins", info.minutes),
		"Funding tiers":                strconv.Itoa(info.tiers),
		"Captha protection":            fmt.Sprintf("%v", info.captchaToken != ""),
		"Using Twitter API":            fmt.Sprintf("%v", info.twitterToken != ""),
		"Bfestats username":            info.node.bfestats,
	}
	if info.noauth {
...
func checkFaucet(client *sshClient, network string) (*faucetInfos, error) {
	...
	return &faucetInfos{
		node: &nodeInfos{
			...
			port:     infos.portmap[infos.envvars["BFE_PORT"]+"/tcp"],
			bfestats: infos.envvars["BFE_NAME"],
			keyJSON:  keyJSON,
			keyPass:  keyPass,
		},
		host:          host,
