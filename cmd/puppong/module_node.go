...
// nodeDockerfile is the Dockerfile required to run an Bfedu node.
var nodeDockerfile = `
FROM bfedu2020/client-go:latest
...
{{end}}
RUN \
  echo 'gbfe --cache 512 init /genesis.json' > gbfe.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.bfedu2020/keystore/ && cp /signer.json /root/.bfedu2020/keystore/' >> gbfe.sh && \{{end}}
	echo $'exec gbfe --networkid {{.NetworkID}} --cache 512 --port {{.Port}} --nat extip:{{.IP}} --maxpeers {{.Peers}} {{.LightFlag}} --bfestats \'{{.Bfestats}}\' {{if .Bootnodes}}--bootnodes {{.Bootnodes}}{{end}} {{if .Bfeerbase}}--miner.bfeerbase {{.Bfeerbase}} --mine --miner.threads 1{{end}} {{if .Unlock}}--unlock 0 --password /signer.pass --mine{{end}} --miner.gastarget {{.GasTarget}} --miner.gaslimit {{.GasLimit}} --miner.gasprice {{.GasPrice}}' >> gbfe.sh

ENTRYPOINT ["/bin/sh", "gbfe.sh"]
`

// nodeComposefile is the docker-compose.yml file required to deploy and maintain
// an Bfedu node (bootnode or miner for now).
var nodeComposefile = `
version: '2'
services:
  {{.Type}}:
    ...
    ports:
      ...
      - "{{.Port}}:{{.Port}}/udp"
    volumes:
      - {{.Datadir}}:/root/.bfedu{{if .Bfeashdir}}
      - {{.Bfeashdir}}:/root/.bfeash{{end}}
    environment:
      - PORT={{.Port}}/tcp
      - TOTAL_PEERS={{.TotalPeers}}
      - LIGHT_PEERS={{.LightPeers}}
      - STATS_NAME={{.Bfestats}}
      - MINER_NAME={{.Bfeerbase}}
...
// deployNode deploys a new Bfedu node container to a remote machine via SSH,
// docker and docker-compose. If an instance with the specified network name
// already exists there, it will be overwritten!
func deployNode(client *sshClient, network string, bootnodes []string, config *nodeInfos, nocache bool) ([]byte, error) {
	kind := "sealnode"
	if config.keyJSON == "" && config.bfeerbase == "" {
		kind = "bootnode"
	...
	template.Must(template.New("").Parse(nodeDockerfile)).Execute(dockerfile, map[string]interface{}{
		...
		"Bfestats":  config.bfestats,
		"Bfeerbase": config.bfeerbase,
	...
	template.Must(template.New("").Parse(nodeComposefile)).Execute(composefile, map[string]interface{}{
		...
		"Bfeashdir":  config.bfeashdir,
		"Network":    network,
		"Port":       config.port,
		"TotalPeers": config.peersTotal,
		"Light":      config.peersLight > 0,
		"LightPeers": config.peersLight,
		"Bfestats":   config.bfestats[:strings.Index(config.bfestats, ":")],
		"Bfeerbase":  config.bfeerbase,
...
func (info *nodeInfos) Report() map[string]string {
	report := map[string]string{
		...
		"Bfestats username":        info.bfestats,
	}
	if info.gasTarget > 0 {
		...
		if info.bfeerbase != "" {
			// Bfeash proof-of-work miner
			report["Bfeash directory"] = info.bfeashdir
			report["Miner account"] = info.bfeerbase
		}
		if info.keyJSON != "" {
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	var out []byte
	if out, err = client.Run(fmt.Sprintf("docker exec %s_%s_1 gbfe --exec admin.nodeInfo.enode --cache=16 attach", network, kind)); err != nil {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.bfedu"],
		bfeashdir:  infos.volumes["/root/.bfeash"],
		port:       port,
