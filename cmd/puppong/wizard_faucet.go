...
func (w *wizard) deployFaucet() {
	...
	fmt.Println()
	fmt.Printf("How many Bfeers to release per request? (default = %d)\n", infos.amount)
	...
	fmt.Println()
	if infos.node.bfestats == "" {
		fmt.Printf("What should the node be called on the stats page?\n")
		infos.node.bfestats = w.readString() + ":" + w.conf.bfestats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.node.bfestats)
		infos.node.bfestats = w.readDefaultString(infos.node.bfestats) + ":" + w.conf.bfestats
	}
