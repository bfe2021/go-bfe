...
func (w *wizard) makeGenesis() {
	...
	fmt.Println(" 1. Bfeash - proof-of-work")
	fmt.Println(" 2. Clique - proof-of-authority")

	choice := w.read()
	switch {
	case choice == "1":
		// In case of bfeash, we're pretty much done
		genesis.Config.Bfeash = new(params.BfeashConfig)
...
// importGenesis imports a Gbfe genesis spec into puppong.
...
func (w *wizard) manageGenesis() {
	...
	case "2":
		...
		// Export the native genesis spec used by puppong and Gbfe
		gbfeJson := filepath.Join(folder, fmt.Sprintf("%s.json", w.network))
		if err := ioutil.WriteFile(gbfeJson, out, 0644); err != nil {
		...
		// Export the genesis spec used by Harmony (formerly BfeduJ)
