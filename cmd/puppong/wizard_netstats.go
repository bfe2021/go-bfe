...
func (w *wizard) gatherStats(server string, pubkey []byte, client *sshClient) *serverStat {
	...
	logger.Debug("Checking for bfestats availability")
	if infos, err := checkBfestats(client, w.network); err != nil {
		if err != ErrServiceUnknown {
			stat.services["bfestats"] = map[string]string{"offline": err.Error()}
		}
	} else {
		stat.services["bfestats"] = infos.Report()
		bfestats = infos.config
	}
