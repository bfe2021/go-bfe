...
func (w *wizard) deployNode(boot bool) {
	...
	if w.conf.bfestats == "" {
		log.Error("No bfestats server configured")
		return
	}
	...
	if w.conf.Genesis.Config.Bfeash != nil && !boot {
		fmt.Println()
		if infos.bfeashdir == "" {
			fmt.Printf("Where should the bfeash mining DAGs be stored on the remote machine?\n")
			infos.bfeashdir = w.readString()
		} else {
			fmt.Printf("Where should the bfeash mining DAGs be stored on the remote machine? (default = %s)\n", infos.bfeashdir)
			infos.bfeashdir = w.readDefaultString(infos.bfeashdir)
		}
	}
	...
	if infos.bfestats == "" {
		...
		infos.bfestats = w.readString() + ":" + w.conf.bfestats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.bfestats)
		infos.bfestats = w.readDefaultString(infos.bfestats) + ":" + w.conf.bfestats
	}
			...
			if infos.bfeerbase == "" {
				...
				for {
					if address := w.readAddress(); address != nil {
						infos.bfeerbase = address.Hex()
						break
					}
				}
			} else {
				fmt.Printf("What address should the miner use? (default = %s)\n", infos.bfeerbase)
				infos.bfeerbase = w.readDefaultAddress(common.HexToAddress(infos.bfeerbase)).Hex()
			}
	...
	if out, err := deployNode(client, w.network, w.conf.bootnodes, infos, nocache); err != nil {
		log.Error("Failed to deploy Bfedu node container", "err", err)
		if len(out) > 0 {
			fmt.Printf("%s\n", out)
		}
		return
