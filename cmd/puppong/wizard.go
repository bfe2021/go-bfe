...
type config struct {
	...
	bfestats  string   // Bfestats settings to cache for node deploys
...
type wizard struct {
	...
	services map[string][]string   // Bfedu services known to be running on servers
...
// converts it to an Bfedu address. If an empty line is entered, the default
// value is returned.
...
// weird formats used by bfestats, which compares IPs textually, not by value.
