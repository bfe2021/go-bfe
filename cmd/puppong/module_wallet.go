...
RUN \
  echo 'node server.js &'                     > wallet.sh && \
	echo 'gbfe --cache 512 init /genesis.json' >> wallet.sh && \
	echo $'exec gbfe --networkid {{.NetworkID}} --port {{.NodePort}} --bootnodes {{.Bootnodes}} --bfestats \'{{.Bfestats}}\' --cache=512 --http --http.addr=0.0.0.0 --http.corsdomain "*" --http.vhosts "*"' >> wallet.sh

RUN \
	sed -i 's/PuppongNetworkID/{{.NetworkID}}/g' dist/js/bfeerwallet-master.js && \
	sed -i 's/PuppongNetwork/{{.Network}}/g'     dist/js/bfeerwallet-master.js && \
	sed -i 's/PuppongDenom/{{.Denom}}/g'         dist/js/bfeerwallet-master.js && \
	sed -i 's/PuppongHost/{{.Host}}/g'           dist/js/bfeerwallet-master.js && \
	sed -i 's/PuppongRPCPort/{{.RPCPort}}/g'     dist/js/bfeerwallet-master.js
