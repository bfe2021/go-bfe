...
func TestMessageSignVerify(t *testing.T) {
	tmpdir, err := ioutil.TempDir("", "bfekey-test")
	if err != nil {
	...
	// Create the key.
	generate := runBfekey(t, "generate", "--lightkdf", keyfile)
	generate.Expect(`
...
`)
	...
	// Sign a message.
	sign := runBfekey(t, "signmessage", keyfile, message)
	sign.Expect(`
!! Unsupported terminal, password will be echoed.
Password: {{.InputLine "foobar"}}
`)
	...
	// Verify the message.
	verify := runBfekey(t, "verifymessage", address, signature, message)
