...
import (
	...
	"github.com/bfe2021/go-bfe/contracts/checkpointoracle"
...
func newClient(ctx *cli.Context) *bfeclient.Client {
	client, err := bfeclient.Dial(ctx.GlobalString(nodeURLFlag.Name))
	if err != nil {
		utils.Fatalf("Failed to connect to Bfedu node: %v", err)
	}
	return client
}
...
func newContract(client *rpc.Client) (common.Address, *checkpointoracle.CheckpointOracle) {
	...
	contract, err := checkpointoracle.NewCheckpointOracle(addr, bfeclient.NewClient(client))
	if err != nil {
