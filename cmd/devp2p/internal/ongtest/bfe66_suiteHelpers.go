...
func (c *Conn) write66(req bfe.Packet, code int) error {
...
func (c *Conn) waitForBlock66(block *types.Block) error {
	...
	c.SetReadDeadline(timeout)
	for {
		req := bfe.GetBlockHeadersPacket66{
			RequestId: 54,
			GetBlockHeadersPacket: &bfe.GetBlockHeadersPacket{
				Origin: bfe.HashOrNumber{
					Hash: block.Hash(),
				},
				Amount: 1,
			},
...
func (s *Suite) getBlockHeaders66(t *utesting.T, conn *Conn, req bfe.Packet, expectedID uint64) BlockHeaders {
