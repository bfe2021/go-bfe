...
import (
	...
	"github.com/bfe2021/go-bfe/internal/jsre"
	"github.com/bfe2021/go-bfe/internal/jsre/deps"
	"github.com/bfe2021/go-bfe/internal/web3ext"
...
type Config struct {
	...
	Client   *rpc.Client         // RPC client to execute Bfedu requests through
...
func (c *Console) AutoCompleteInput(line string, pos int) (string, []string, string) {
	...
	// E.g. in case of nested lines bfe.getBalance(bfe.coinb<tab><tab>
	start := pos - 1
	for ; start > 0; start-- {
...
// Welcome show summary of current Gbfe instance and some metadata about the
// console's available modules.
func (c *Console) Welcome() {
	message := "Welcome to the Gbfe JavaScript console!\n\n"

	// Print some generic Gbfe metadata
	if res, err := c.jsre.Run(`
		var message = "instance: " + web3.version.node + "\n";
		try {
			message += "coinbase: " + bfe.coinbase + "\n";
		} catch (err) {}
		message += "at block: " + bfe.blockNumber + " (" + new Date(1000 * bfe.getBlock(bfe.blockNumber).timestamp) + ")\n";
		try {
