...
import (
	...
	"github.com/bfe2021/go-bfe/accounts/abi/bind/backends"
	...
	"github.com/bfe2021/go-bfe/contracts/checkpointoracle/contract"
	...
	"github.com/bfe2021/go-bfe/les/checkpointoracle"
	"github.com/bfe2021/go-bfe/les/flowcontrol"
	"github.com/bfe2021/go-bfe/light"
	"github.com/bfe2021/go-bfe/p2p"
	"github.com/bfe2021/go-bfe/p2p/enode"
	"github.com/bfe2021/go-bfe/p2p/nodestate"
...
func prepare(n int, backend *backends.SimulatedBackend) {
	...
	for i := 0; i < n; i++ {
		...
		case 0:
			...
			// bankUser transfers some bfeer to user1
		...
		case 1:
			...
			// bankUser transfers more bfeer to user1
		...
		case 2:
			...
			// bankUser transfer some bfeer to signer
			bankNonce, _ := backend.PendingNonceAt(ctx, bankAddr)
...
func testIndexers(db bfedb.Database, odr light.OdrBackend, config *light.IndexerConfig, disablePruning bool) []*core.ChainIndexer {
...
func newTestClientHandler(backend *backends.SimulatedBackend, odr *LesOdr, indexers []*core.ChainIndexer, db bfedb.Database, peers *serverPeerSet, ulcServers []string, ulcFraction int) *clientHandler {
	var (
	...
	client := &LightBfedu{
		lesCommons: lesCommons{
			...
			config:      &bfeconfig.Config{LightPeers: 100, NetworkId: NetworkId},
			chainConfig: params.AllBfeashProtocolChanges,
...
func newTestServerHandler(blocks int, indexers []*core.ChainIndexer, db bfedb.Database, clock mclock.Clock) (*serverHandler, *backends.SimulatedBackend) {
	var (
		gspec = core.Genesis{
			Config:   params.AllBfeashProtocolChanges,
			Alloc:    core.GenesisAlloc{bankAddr: {Balance: bankFunds}},
