...
func TestReplication(t *testing.T) {
	...
	vals := []struct{ k, v string }{
		...
		{"doge", "coin"},
		{"dog", "puppy"},
		{"somogningveryoddindeedthis is", "myothernodedata"},
	}
	for _, val := range vals {
		updateString(trie, val.k, val.v)
	}
...
func (s *spongeDb) NewBatch() bfedb.Batch                    { return &spongeBatch{s} }
...
func (s *spongeDb) NewIterator(prefix []byte, start []byte) bfedb.Iterator { panic("implement me") }
...
func (b *spongeBatch) Replay(w bfedb.KeyValueWriter) error { return nil }
