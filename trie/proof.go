...
func VerifyProof(rootHash common.Hash, key []byte, proofDb bfedb.KeyValueReader) (value []byte, err error) {
	key = keybytesToHex(key)
	wantHash := rootHash
	for i := 0; ; i++ {
...
func proofToPath(rootHash common.Hash, root node, key []byte, proofDb bfedb.KeyValueReader, allowNonExistent bool) (node, []byte, error) {
...
func VerifyRangeProof(rootHash common.Hash, firstKey []byte, lastKey []byte, keys [][]byte, values [][]byte, proof bfedb.KeyValueReader) (bfedb.KeyValueStore, *Trie, *KeyValueNotary, bool, error) {
