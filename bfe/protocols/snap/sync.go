...
type accountResponse struct {
	...
	nodes bfedb.KeyValueStore // Database containing the reconstructed trie nodes
...
// Syncer is an Bfedu account and storage trie syncer based on snapshots and
...
type Syncer struct {
	db    bfedb.KeyValueStore // Database to store the trie nodes into (and dedup)
...
// NewSyncer creates a new snapshot syncer to download the Bfedu state over the
// snap protocol.
func NewSyncer(db bfedb.KeyValueStore, bloom *trie.SyncBloom) *Syncer {
	return &Syncer{
		db:    db,
		bloom: bloom,
...
func (s *Syncer) OnStorage(peer SyncPeer, id uint64, hashes [][]common.Hash, slots [][][]byte, proof [][]byte) error {
	...
	var (
		dbs    = make([]bfedb.KeyValueStore, len(hashes))
