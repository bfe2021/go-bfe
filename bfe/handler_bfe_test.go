...
func (h *testBfeHandler) Chain() *core.BlockChain              { panic("no backing chain") }
func (h *testBfeHandler) StateBloom() *trie.SyncBloom          { panic("no backing state bloom") }
func (h *testBfeHandler) TxPool() bfe.TxPool                   { panic("no backing tx pool") }
func (h *testBfeHandler) AcceptTxs() bool                      { return true }
func (h *testBfeHandler) RunPeer(*bfe.Peer, bfe.Handler) error { panic("not used in tests") }
func (h *testBfeHandler) PeerInfo(enode.ID) interface{}        { panic("not used in tests") }

func (h *testBfeHandler) Handle(peer *bfe.Peer, packet bfe.Packet) error {
	switch packet := packet.(type) {
	case *bfe.NewBlockPacket:
		h.blockBroadcasts.Send(packet.Block)
		return nil

	case *bfe.NewPooledTransactionHashesPacket:
		h.txAnnounces.Send(([]common.Hash)(*packet))
		return nil

	case *bfe.TransactionsPacket:
		h.txBroadcasts.Send(([]*types.Transaction)(*packet))
		return nil

	case *bfe.PooledTransactionsPacket:
		h.txBroadcasts.Send(([]*types.Transaction)(*packet))
		return nil
...
func testForkIDSplit(t *testing.T, protocol uint) {
	...
	peerNoFork := bfe.NewPeer(protocol, p2p.NewPeer(enode.ID{1}, "", nil), p2pNoFork, nil)
	peerProFork := bfe.NewPeer(protocol, p2p.NewPeer(enode.ID{2}, "", nil), p2pProFork, nil)
...
func testRecvTransactions(t *testing.T, protocol uint) {
	...
	src := bfe.NewPeer(protocol, p2p.NewPeer(enode.ID{1}, "", nil), p2pSrc, handler.txpool)
	sink := bfe.NewPeer(protocol, p2p.NewPeer(enode.ID{2}, "", nil), p2pSink, handler.txpool)
	defer src.Close()
	defer sink.Close()
...
func testSendTransactions(t *testing.T, protocol uint) {
	...
	for len(seen) < len(insert) {
		...
		case 63, 64:
			select {
			case <-anns:
				t.Errorf("tx announce received on pre bfe/65")
			case txs := <-bcasts:
				for _, tx := range txs {
		...
		case 65:
			...
			case <-bcasts:
				t.Errorf("initial tx broadcast received on post bfe/65")
			}
...
func testTransactionPropagation(t *testing.T, protocol uint) {
	...
	for i, sink := range sinks {
		...
		sourcePeer := bfe.NewPeer(protocol, p2p.NewPeer(enode.ID{byte(i)}, "", nil), sourcePipe, source.txpool)
		sinkPeer := bfe.NewPeer(protocol, p2p.NewPeer(enode.ID{0}, "", nil), sinkPipe, sink.txpool)
...
func testCheckpointChallenge(t *testing.T, syncmode downloader.SyncMode, checkpoint bool, timeout bool, empty bool, match bool, drop bool) {
	...
	local := bfe.NewPeer(bfe.BFE32, p2p.NewPeer(enode.ID{1}, "", nil), p2pLocal, handler.txpool)
	remote := bfe.NewPeer(bfe.BFE32, p2p.NewPeer(enode.ID{2}, "", nil), p2pRemote, handler.txpool)
	defer local.Close()
	defer remote.Close()
...
func testBroadcastBlock(t *testing.T, peers, bcasts int) {
	...
	sinks := make([]*testBfeHandler, peers)
	for i := 0; i < len(sinks); i++ {
		sinks[i] = new(testBfeHandler)
	}
	// Interconnect all the sink handlers with the source handler
	var (
	...
	for i, sink := range sinks {
		...
		sourcePeer := bfe.NewPeer(bfe.BFE32, p2p.NewPeer(enode.ID{byte(i)}, "", nil), sourcePipe, nil)
		sinkPeer := bfe.NewPeer(bfe.BFE32, p2p.NewPeer(enode.ID{0}, "", nil), sinkPipe, nil)
		defer sourcePeer.Close()
		defer sinkPeer.Close()
...
func testBroadcastMalformedBlock(t *testing.T, protocol uint) {
	...
	src := bfe.NewPeer(protocol, p2p.NewPeer(enode.ID{1}, "", nil), p2pSrc, source.txpool)
	sink := bfe.NewPeer(protocol, p2p.NewPeer(enode.ID{2}, "", nil), p2pSink, source.txpool)
	defer src.Close()
	defer sink.Close()
