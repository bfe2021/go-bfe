...
// PublicBfeduAPI provides an API to access Bfedu full node-related
// information.
type PublicBfeduAPI struct {
	e *Bfedu
}

// NewPublicBfeduAPI creates a new Bfedu protocol API for full nodes.
func NewPublicBfeduAPI(e *Bfedu) *PublicBfeduAPI {
	return &PublicBfeduAPI{e}
}

// Bfeerbase is the address that mining rewards will be send to
func (api *PublicBfeduAPI) Bfeerbase() (common.Address, error) {
	return api.e.Bfeerbase()
}

// Coinbase is the address that mining rewards will be send to (alias for Bfeerbase)
func (api *PublicBfeduAPI) Coinbase() (common.Address, error) {
	return api.Bfeerbase()
}

// Hashrate returns the POW hashrate
func (api *PublicBfeduAPI) Hashrate() hexutil.Uint64 {
	return hexutil.Uint64(api.e.Miner().HashRate())
}

// ChainId is the EIP-155 replay-protection chain id for the current bfedu chain config.
func (api *PublicBfeduAPI) ChainId() (hexutil.Uint64, error) {
...
func NewPublicMinerAPI(e *Bfedu) *PublicMinerAPI {
...
func NewPrivateMinerAPI(e *Bfedu) *PrivateMinerAPI {
...
func (api *PrivateAdminAPI) ExportChain(file string, first *uint64, last *uint64) (bool, error) {
	...
	if first != nil && last == nil {
		head := api.bfe.BlockChain().CurrentHeader().Number.Uint64()
		last = &head
	}
	...
	if first != nil {
		if err := api.bfe.BlockChain().ExportN(writer, *first, *last); err != nil {
			return false, err
		}
	} else if err := api.bfe.BlockChain().Export(writer); err != nil {
		return false, err
	}
	return true, nil
}
...
func (api *PrivateAdminAPI) ImportChain(file string) (bool, error) {
	...
	for batch := 0; ; batch++ {
		...
		if hasAllBlocks(api.bfe.BlockChain(), blocks) {
			blocks = blocks[:0]
			continue
		}
		// Import the batch and reset the buffer
		if _, err := api.bfe.BlockChain().InsertChain(blocks); err != nil {
...
func (api *PublicDebugAPI) DumpBlock(blockNr rpc.BlockNumber) (state.Dump, error) {
	if blockNr == rpc.PendingBlockNumber {
		...
		_, stateDb := api.bfe.miner.Pending()
		return stateDb.RawDump(false, false, true), nil
	}
	var block *types.Block
	if blockNr == rpc.LatestBlockNumber {
		block = api.bfe.blockchain.CurrentBlock()
	} else {
		block = api.bfe.blockchain.GetBlockByNumber(uint64(blockNr))
	}
	if block == nil {
		return state.Dump{}, fmt.Errorf("block #%d not found", blockNr)
	}
	stateDb, err := api.bfe.BlockChain().StateAt(block.Root())
	if err != nil {
...
func (api *PrivateDebugAPI) Preimage(ctx context.Context, hash common.Hash) (hexutil.Bytes, error) {
	if preimage := rawdb.ReadPreimage(api.bfe.ChainDb(), hash); preimage != nil {
		return preimage, nil
	}
...
func (api *PrivateDebugAPI) GetBadBlocks(ctx context.Context) ([]*BadBlockArgs, error) {
	var (
		err     error
		blocks  = rawdb.ReadAllBadBlocks(api.bfe.chainDb)
	...
	for _, block := range blocks {
		...
		if blockJSON, err = bfeapi.RPCMarshalBlock(block, true, true); err != nil {
	...
	if number, ok := blockNrOrHash.Number(); ok {
		if number == rpc.PendingBlockNumber {
			...
			if number == rpc.LatestBlockNumber {
				...
				block = api.bfe.blockchain.GetBlockByNumber(uint64(number))
			}
			if block == nil {
		...
		block := api.bfe.blockchain.GetBlockByHash(hash)
		if block == nil {
...
func (api *PrivateDebugAPI) StorageRangeAt(blockHash common.Hash, txIndex int, contractAddress common.Address, keyStart hexutil.Bytes, maxResult int) (StorageRangeResult, error) {
	// Retrieve the block
	block := api.bfe.blockchain.GetBlockByHash(blockHash)
	if block == nil {
		return StorageRangeResult{}, fmt.Errorf("block %#x not found", blockHash)
	}
	_, _, statedb, release, err := api.bfe.stateAtTransaction(block, txIndex, 0)
	if err != nil {
...
func (api *PrivateDebugAPI) GetModifiedAccountsByNumber(startNum uint64, endNum *uint64) ([]common.Address, error) {
	...
	startBlock = api.bfe.blockchain.GetBlockByNumber(startNum)
	if startBlock == nil {
	...
	if endNum == nil {
		endBlock = startBlock
		startBlock = api.bfe.blockchain.GetBlockByHash(startBlock.ParentHash())
		if startBlock == nil {
			return nil, fmt.Errorf("block %x has no parent", endBlock.Number())
		}
	} else {
		endBlock = api.bfe.blockchain.GetBlockByNumber(*endNum)
		if endBlock == nil {
...
func (api *PrivateDebugAPI) GetModifiedAccountsByHash(startHash common.Hash, endHash *common.Hash) ([]common.Address, error) {
	var startBlock, endBlock *types.Block
	startBlock = api.bfe.blockchain.GetBlockByHash(startHash)
	if startBlock == nil {
	...
	if endHash == nil {
		...
		endBlock = api.bfe.blockchain.GetBlockByHash(*endHash)
		if endBlock == nil {
...
func (api *PrivateDebugAPI) getModifiedAccounts(startBlock, endBlock *types.Block) ([]common.Address, error) {
	...
	triedb := api.bfe.BlockChain().StateCache().TrieDB()
