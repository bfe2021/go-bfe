...
import (
	...
	"github.com/bfe2021/go-bfe/bfe/fetcher"
...
// support all the operations needed by the Bfedu chain protocols.
...
type handlerConfig struct {
	Database   bfedb.Database            // Database for direct sync insertions
...
// newHandler returns a handler for all Bfedu chain management protocol.
...
func (h *handler) runBfePeer(peer *bfe.Peer, handler bfe.Handler) error {
	...
	if err := peer.Handshake(h.networkID, td, hash, genesis.Hash(), forkID, h.forkFilter); err != nil {
		peer.Log().Debug("Bfedu handshake failed", "err", err)
		return err
	}
	...
	peer.Log().Debug("Bfedu peer connected", "name", peer.Name())

	// Register the peer locally
	if err := h.peers.registerPeer(peer, snap); err != nil {
		peer.Log().Error("Bfedu peer registration failed", "err", err)
		return err
	}
...
func (h *handler) removePeer(id string) {
	...
	if err := h.peers.unregisterPeer(id); err != nil {
		logger.Error("Bfedu peer removal failed", "err", err)
	}
...
func (h *handler) Start(maxPeers int) {
	...
	h.wg.Add(2)
	go h.chainSync.loop()
	go h.txsyncLoop64() // TODO(karalabe): Legacy initial tx echange, drop with bfe/64.
}
...
func (h *handler) Stop() {
	...
	log.Info("Bfedu protocol stopped")
}
