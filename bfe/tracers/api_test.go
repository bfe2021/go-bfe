...
func TestTraceCall(t *testing.T) {
	...
	genesis := &core.Genesis{Alloc: core.GenesisAlloc{
		accounts[0].addr: {Balance: big.NewInt(params.Bfeer)},
		accounts[1].addr: {Balance: big.NewInt(params.Bfeer)},
		accounts[2].addr: {Balance: big.NewInt(params.Bfeer)},
	}}
	genBlocks := 10
	...
	var testSuite = []struct {
		...
		// Standard JSON trace upon the genesis, plain transfer.
		{
			...
			config:    nil,
			expectErr: nil,
			expect: &bfeapi.ExecutionResult{
				Gas:         params.TxGas,
				Failed:      false,
				ReturnValue: "",
				StructLogs:  []bfeapi.StructLogRes{},
			},
		},
...
func TestTraceTransaction(t *testing.T) {
	...
	if !reflect.DeepEqual(result, &bfeapi.ExecutionResult{
		Gas:         params.TxGas,
		Failed:      false,
		ReturnValue: "",
		StructLogs:  []bfeapi.StructLogRes{},
