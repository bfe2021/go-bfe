...
import (
	...
	"github.com/bfe2021/go-bfe/tests"
)

// To generate a new callTracer test, copy paste the makeTest Method below into
// a Gbfe console and call it with a transaction hash you which to export.
...
var makeTest = function(tx, rewind) {
  ...
  var block   = bfe.getBlock(bfe.getTransaction(tx).blockHash);
  var genesis = bfe.getBlock(block.parentHash);
  ...
  genesis.config = admin.nodeInfo.protocols.bfe.config;
  ...
  console.log(JSON.stringify({
    ...
    input:  bfe.getRawTransaction(tx),
    result: result,
  }, null, 2));
}
*/
