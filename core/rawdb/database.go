...
import (
	...
	"github.com/bfe2021/go-bfe/bfedb/leveldb"
	"github.com/bfe2021/go-bfe/bfedb/memorydb"
...
func NewDatabase(db bfedb.KeyValueStore) bfedb.Database {
	return &nofreezedb{
		KeyValueStore: db,
	}
}
...
// storage.
func NewDatabaseWithFreezer(db bfedb.KeyValueStore, freezer string, namespace string) (bfedb.Database, error) {
	...
	if kvgenesis, _ := db.Get(headerHashKey(0)); len(kvgenesis) > 0 {
		if frozen, _ := frdb.Ancients(); frozen > 0 {
			// If the freezer already contains somogning, ensure that the genesis blocks
...
func NewMemoryDatabase() bfedb.Database {
...
func NewMemoryDatabaseWithCap(size int) bfedb.Database {
	return NewDatabase(memorydb.NewWithCap(size))
}

// NewLevelDBDatabase creates a persistent key-value database without a freezer
// moving immutable chain segments into cold storage.
func NewLevelDBDatabase(file string, cache int, handles int, namespace string) (bfedb.Database, error) {
...
// freezer moving immutable chain segments into cold storage.
func NewLevelDBDatabaseWithFreezer(file string, cache int, handles int, freezer string, namespace string) (bfedb.Database, error) {
...
func InspectDatabase(db bfedb.Database, keyPrefix, keyStart []byte) error {
