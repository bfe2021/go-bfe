...
func ReadSnapshotRoot(db bfedb.KeyValueReader) common.Hash {
...
// the persisted snapshot.
func WriteSnapshotRoot(db bfedb.KeyValueWriter, root common.Hash) {
...
// invalid.
func DeleteSnapshotRoot(db bfedb.KeyValueWriter) {
...
func ReadAccountSnapshot(db bfedb.KeyValueReader, hash common.Hash) []byte {
	data, _ := db.Get(accountSnapshotKey(hash))
	return data
}

// WriteAccountSnapshot stores the snapshot entry of an account trie leaf.
func WriteAccountSnapshot(db bfedb.KeyValueWriter, hash common.Hash, entry []byte) {
...
func DeleteAccountSnapshot(db bfedb.KeyValueWriter, hash common.Hash) {
...
func ReadStorageSnapshot(db bfedb.KeyValueReader, accountHash, storageHash common.Hash) []byte {
	data, _ := db.Get(storageSnapshotKey(accountHash, storageHash))
	return data
}

// WriteStorageSnapshot stores the snapshot entry of an storage trie leaf.
func WriteStorageSnapshot(db bfedb.KeyValueWriter, accountHash, storageHash common.Hash, entry []byte) {
...
func DeleteStorageSnapshot(db bfedb.KeyValueWriter, accountHash, storageHash common.Hash) {
...
func IterateStorageSnapshots(db bfedb.Iteratee, accountHash common.Hash) bfedb.Iterator {
	return db.NewIterator(storageSnapshotsKey(accountHash), nil)
}

// ReadSnapshotJournal retrieves the serialized in-memory diff layers saved at
// the last shutdown. The blob is expected to be max a few 10s of megabytes.
func ReadSnapshotJournal(db bfedb.KeyValueReader) []byte {
...
func WriteSnapshotJournal(db bfedb.KeyValueWriter, journal []byte) {
...
// the last shutdown
func DeleteSnapshotJournal(db bfedb.KeyValueWriter) {
...
// the last shutdown.
func ReadSnapshotGenerator(db bfedb.KeyValueReader) []byte {
...
// shutdown.
func WriteSnapshotGenerator(db bfedb.KeyValueWriter, generator []byte) {
...
// the last shutdown
func DeleteSnapshotGenerator(db bfedb.KeyValueWriter) {
...
// snapshot layer.
func ReadSnapshotRecoveryNumber(db bfedb.KeyValueReader) *uint64 {
...
// snapshot layer.
func WriteSnapshotRecoveryNumber(db bfedb.KeyValueWriter, number uint64) {
	var buf [8]byte
...
// snapshot layer.
func DeleteSnapshotRecoveryNumber(db bfedb.KeyValueWriter) {
...
func ReadSnapshotSyncStatus(db bfedb.KeyValueReader) []byte {
	data, _ := db.Get(snapshotSyncStatusKey)
	return data
}

// WriteSnapshotSyncStatus stores the serialized sync status to save at shutdown.
func WriteSnapshotSyncStatus(db bfedb.KeyValueWriter, status []byte) {
...
// shutdown
func DeleteSnapshotSyncStatus(db bfedb.KeyValueWriter) {
