...
func ReadDatabaseVersion(db bfedb.KeyValueReader) *uint64 {
	var version uint64
...
func WriteDatabaseVersion(db bfedb.KeyValueWriter, version uint64) {
...
func ReadChainConfig(db bfedb.KeyValueReader, hash common.Hash) *params.ChainConfig {
...
func WriteChainConfig(db bfedb.KeyValueWriter, hash common.Hash, cfg *params.ChainConfig) {
	if cfg == nil {
		return
	}
...
func PushUncleanShutdownMarker(db bfedb.KeyValueStore) ([]uint64, uint64, error) {
...
func PopUncleanShutdownMarker(db bfedb.KeyValueStore) {
	var uncleanShutdowns crashList
	// Read old data
	if data, err := db.Get(uncleanShutdownKey); err != nil {
		log.Warn("Error reading unclean shutdown markers", "error", err)
