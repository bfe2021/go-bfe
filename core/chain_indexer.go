...
type ChainIndexer struct {
	chainDb  bfedb.Database      // Chain database to index the data from
	indexDb  bfedb.Database      // Prefixed table-view of the db to write index metadata into
...
func NewChainIndexer(chainDb bfedb.Database, indexDb bfedb.Database, backend ChainIndexerBackend, section, confirm uint64, throttling time.Duration, kind string) *ChainIndexer {
	c := &ChainIndexer{
		chainDb:     chainDb,
		indexDb:     indexDb,
		backend:     backend,
