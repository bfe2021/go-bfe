...
type BlockContext struct {
	...
	// sufficient bfeer to transfer the value
	CanTransfer CanTransferFunc
	// Transfer transfers bfeer from one account to the other
...
// EVM is the Bfedu Virtual Machine base object and provides
...
type EVM struct {
	...
	// evm.
	vmConfig Config
	// global (to this context) bfedu virtual machine
...
func (evm *EVM) CallCode(caller ContractRef, addr common.Address, input []byte, gas uint64, value *big.Int) (ret []byte, leftOverGas uint64, err error) {
	...
	// Fail if we're trying to transfer more than the available balance
	// Note although it's noop to transfer X bfeer to caller itself. But
...
func (evm *EVM) StaticCall(caller ContractRef, addr common.Address, input []byte, gas uint64) (ret []byte, leftOverGas uint64, err error) {
	...
	// This doesn't matter on Mainnet, where all empties are gbfe at the time of Byzantium,
