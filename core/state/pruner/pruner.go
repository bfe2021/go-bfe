...
func NewPruner(db bfedb.Database, headHeader *types.Header, datadir, trieCachePath string, bloomSize uint64) (*Pruner, error) {
...
func prune(maindb bfedb.Database, stateBloom *stateBloom, middleStateRoots map[common.Hash]struct{}, start time.Time) error {
...
// in the disk.
func RecoverPruning(datadir string, db bfedb.Database, trieCachePath string) error {
...
func extractGenesis(db bfedb.Database, stateBloom *stateBloom) error {
...
func getHeadHeader(db bfedb.Database) (*types.Header, error) {
...
pruning. Remember don't start the Gbfe without deleting the clean trie cache
otherwise the entire database may be damaged!

Check the command description "gbfe snapshot prune-state --help" for more details.
`
