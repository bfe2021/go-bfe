...
func loadAndParseLegacyJournal(db bfedb.KeyValueStore, base *diskLayer) (snapshot, journalGenerator, error) {
...
func loadAndParseJournal(db bfedb.KeyValueStore, base *diskLayer) (snapshot, journalGenerator, error) {
	...
	// not existent, e.g. the disk layer is generating while that the Gbfe
	...
	// It can happen that Gbfe crashes without persisting the latest
	// diff journal.
...
func loadSnapshot(diskdb bfedb.KeyValueStore, triedb *trie.Database, cache int, root common.Hash, recovery bool) (snapshot, error) {
	...
	//
	// Possible scenario: Gbfe was crashed without persisting journal and then
