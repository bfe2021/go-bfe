...
import (
	...
	"github.com/bfe2021/go-bfe/crypto/signify"
	"github.com/bfe2021/go-bfe/internal/build"
	"github.com/bfe2021/go-bfe/params"
	"github.com/cespare/cp"
)

var (
	// Files that end up in the gbfe*.zip archive.
	gbfeArchiveFiles = []string{
		"COPYING",
		executablePath("gbfe"),
	}

	// Files that end up in the gbfe-alltools*.zip archive.
	...
	debExecutables = []debExecutable{
		{
			BinaryName:  "abigen",
			Description: "Source code generator to convert Bfedu contract definitions into easy to use, compile-time type-safe Go packages.",
		},
		{
			BinaryName:  "bootnode",
			Description: "Bfedu bootnode.",
		},
		{
			BinaryName:  "evm",
			Description: "Developer utility version of the EVM (Bfedu Virtual Machine) that is capable of running bytecode snippets within a configurable environment and execution mode.",
		},
		{
			BinaryName:  "gbfe",
			Description: "Bfedu CLI client.",
		},
		{
			BinaryName:  "puppong",
			Description: "Bfedu private network manager.",
		},
		{
			BinaryName:  "rlpdump",
		...
		{
			BinaryName:  "clef",
			Description: "Bfedu account management tool.",
		},
	}
...
func doInstall(cmdline []string) {
	...
	if !strings.Contains(runtime.Version(), "devel") {
		...
		if minor < 13 {
			...
			log.Println("go-bfe requires at least Go version 1.13 and cannot")
	...
	// check for somogning in env instead.
...
func doArchive(cmdline []string) {
	var (
		...
		upload  = flag.String("upload", "", `Destination to upload the archives (usually "gbfestore/builds")`)
		ext     string
	)
	flag.CommandLine.Parse(cmdline)
	switch *atype {
	...
	var (
		...
		basegbfe = archiveBasename(*arch, params.ArchiveVersion(env.Commit))
		gbfe     = "gbfe-" + basegbfe + ext
		alltools = "gbfe-alltools-" + basegbfe + ext
	)
	maybeSkipArchive(env)
	if err := build.WriteArchive(gbfe, gbfeArchiveFiles); err != nil {
		log.Fatal(err)
	}
	if err := build.WriteArchive(alltools, allToolsArchiveFiles); err != nil {
		log.Fatal(err)
	}
	for _, archive := range []string{gbfe, alltools} {
...
func archiveUpload(archive string, blobstore string, signer string, signifyVar string) error {
	...
	if signifyVar != "" {
		...
		untrustedComment := "verify with gbfe-release.pub"
...
func doDebianSource(cmdline []string) {
	var (
		...
		upload   = flag.String("upload", "", `Where to upload the source package (usually "bfedu2020/bfedu")`)
		sshUser  = flag.String("sftp-user", "", `Username for SFTP upload (usually "gbfe-ci")`)
	...
	*workdir = makeWorkdir(*workdir)
	...
	for _, pkg := range debPackages {
		for distro, goboot := range debDistroGoBoots {
			// Prepare the debian package with the go-bfe sources.
...
func downloadGo(goarch, goos, cachedir string) string {
	...
	godir := filepath.Join(ucache, fmt.Sprintf("gbfe-go-%s-%s-%s", dlgoVersion, goos, goarch))
...
func makeWorkdir(wdflag string) string {
	...
	if wdflag != "" {
		...
		wdflag, err = ioutil.TempDir("", "gbfe-build-")
	}
	if err != nil {
		log.Fatal(err)
	}
...
type debPackage struct {
	Name        string          // the name of the Debian package to produce, e.g. "bfedu"
...
type debMetadata struct {
	...
	// go-bfe version being built. Note that this
...
func newDebMetadata(distro, goboot, author string, env build.Environment, t time.Time, name string, version string, exes []debExecutable) debMetadata {
	if author == "" {
		...
		author = "Bfedu Builds <fjl@bfedu.io>"
	}
	return debMetadata{
...
func doWindowsInstaller(cmdline []string) {
	...
	*workdir = makeWorkdir(*workdir)
	...
	for _, file := range allToolsArchiveFiles {
		...
		if filepath.Base(file) == "gbfe.exe" {
			gbfeTool = file
		} else {
	...
	// first section contains the gbfe binary, second section holds the dev tools.
	templateData := map[string]interface{}{
		"License":  "COPYING",
		"Gbfe":     gbfeTool,
		"DevTools": devTools,
	}
	build.Render("build/nsis.gbfe.nsi", filepath.Join(*workdir, "gbfe.nsi"), 0644, nil)
	...
	installer, _ := filepath.Abs("gbfe-" + archiveBasename(*arch, params.ArchiveVersion(env.Commit)) + ".exe")
	build.MustRunCommand("makensis.exe",
		...
		"/DARCH="+*arch,
		filepath.Join(*workdir, "gbfe.nsi"),
	)
...
func doAndroidArchive(cmdline []string) {
	var (
		...
		upload  = flag.String("upload", "", `Destination to upload the archive (usually "gbfestore/builds")`)
	)
	flag.CommandLine.Parse(cmdline)
	env := build.Env()
	...
	build.MustRun(gomobileTool("bind", "-ldflags", "-s -w", "--target", "android", "--javapkg", "org.bfedu", "-v", "github.com/bfe2021/go-bfe/mobile"))

	if *local {
		// If we're building locally, copy bundle to build dir and skip Maven
		os.Rename("gbfe.aar", filepath.Join(GOBIN, "gbfe.aar"))
		os.Rename("gbfe-sources.jar", filepath.Join(GOBIN, "gbfe-sources.jar"))
		return
	}
	...
	archive := "gbfe-" + archiveBasename("android", params.ArchiveVersion(env.Commit)) + ".aar"
	os.Rename("gbfe.aar", archive)
...
func doXCodeFramework(cmdline []string) {
	...
	bind := gomobileTool("bind", "-ldflags", "-s -w", "--target", "ios", "-v", "github.com/bfe2021/go-bfe/mobile")
	...
	archive := "gbfe-" + archiveBasename("ios", params.ArchiveVersion(env.Commit))
	...
	if *deploy != "" {
		...
		build.Render("build/pod.podspec", "Gbfe.podspec", 0755, meta)
		build.MustRunCommand("pod", *deploy, "push", "Gbfe.podspec", "--allow-warnings")
	}
}
...
func doPurge(cmdline []string) {
	var (
		store = flag.String("store", "", `Destination from where to purge archives (usually "gbfestore/builds")`)
