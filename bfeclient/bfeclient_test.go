...
// Verify that Client implements the bfedu interfaces.
var (
	_ = bfedu.ChainReader(&Client{})
	_ = bfedu.TransactionReader(&Client{})
	_ = bfedu.ChainStateReader(&Client{})
	_ = bfedu.ChainSyncReader(&Client{})
	_ = bfedu.ContractCaller(&Client{})
	_ = bfedu.GasEstimator(&Client{})
	_ = bfedu.GasPricer(&Client{})
	_ = bfedu.LogFilterer(&Client{})
	_ = bfedu.PendingStateReader(&Client{})
	// _ = bfedu.PendingStateEventer(&Client{})
	_ = bfedu.PendingContractCaller(&Client{})
)
...
func newTestBackend(t *testing.T) (*node.Node, []*types.Block) {
	...
	// Create Bfedu Service
	config := &bfeconfig.Config{Genesis: genesis}
	config.Bfeash.PowMode = bfeash.ModeFake
	bfeservice, err := bfe.New(n, config)
	if err != nil {
		t.Fatalf("can't create new bfedu service: %v", err)
	}
	// Import the test chain.
	if err := n.Start(); err != nil {
		t.Fatalf("can't start test node: %v", err)
	}
	if _, err := bfeservice.BlockChain().InsertChain(blocks[1:]); err != nil {
...
func TestBfeClient(t *testing.T) {
...
func testTransactionInBlockInterrupted(t *testing.T, client *rpc.Client) {
	...
	if err == nil || err == bfedu.NotFound {
		t.Fatal("error should not be nil/notfound")
	}
	// Test tx in block not found
	if _, err := ec.TransactionInBlock(context.Background(), block.Hash(), 1); err != bfedu.NotFound {
		t.Fatal("error should be bfedu.NotFound")
	}
}
...
func testChainID(t *testing.T, client *rpc.Client) {
	...
	if id == nil || id.Cmp(params.AllBfeashProtocolChanges.ChainID) != 0 {
