...
const schema string = `
    ...
    scalar Bytes32
    # Address is a 20 byte Bfedu address, represented as 0x-prefixed hexadecimal.
    scalar Address
    ...
    # Account is an Bfedu account at a particular block.
    type Account {
    ...
    # Transaction is an Bfedu transaction.
    type Transaction {
    ...
    type Block {
        ...
        # Account fetches an Bfedu account at the current block's state.
    ...
    type Pending {
      ...
      transactions: [Transaction!]
      # Account fetches an Bfedu account for the pending state.
      account(address: Address!): Account!
    ...
    type Query {
        # Block fetches an Bfedu block by number or by hash. If neither is
