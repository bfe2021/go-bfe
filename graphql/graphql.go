...
// Package graphql provides a GraphQL interface to Bfedu node data.
package graphql
...
func (b *Long) UnmarshalGraphQL(input interface{}) error {
	...
	case string:
		...
		//	// apply leniency and support hex representations of logns.
...
// Account represents an Bfedu account at a particular block.
type Account struct {
	backend       bfeapi.Backend
...
// Transaction represents an Bfedu transaction.
...
// Block represents an Bfedu block.
...
// `Log` objects.
func runFilter(ctx context.Context, be bfeapi.Backend, filter *filters.Filter) ([]*Log, error) {
...
type CallData struct {
	From     *common.Address // The Bfedu address the call is from.
	To       *common.Address // The Bfedu address the call is to.
...
func (b *Block) Call(ctx context.Context, args struct {
	...
	result, err := bfeapi.DoCall(ctx, b.backend, args.Data, *b.numberOrHash, nil, vm.Config{}, 5*time.Second, b.backend.RPCGasCap())
	if err != nil {
		return nil, err
	}
	status := Long(1)
...
func (b *Block) EstimateGas(ctx context.Context, args struct {
	...
	gas, err := bfeapi.DoEstimateGas(ctx, b.backend, args.Data, *b.numberOrHash, b.backend.RPCGasCap())
	return Long(gas), err
}
...
func (p *Pending) Call(ctx context.Context, args struct {
	...
	pendingBlockNr := rpc.BlockNumberOrHashWithNumber(rpc.PendingBlockNumber)
	result, err := bfeapi.DoCall(ctx, p.backend, args.Data, pendingBlockNr, nil, vm.Config{}, 5*time.Second, p.backend.RPCGasCap())
	if err != nil {
		return nil, err
	}
	status := Long(1)
...
func (p *Pending) EstimateGas(ctx context.Context, args struct {
	...
	pendingBlockNr := rpc.BlockNumberOrHashWithNumber(rpc.PendingBlockNumber)
	gas, err := bfeapi.DoEstimateGas(ctx, p.backend, args.Data, pendingBlockNr, p.backend.RPCGasCap())
	return Long(gas), err
}
...
func (r *Resolver) SendRawTransaction(ctx context.Context, args struct{ Data hexutil.Bytes }) (common.Hash, error) {
	...
	hash, err := bfeapi.SubmitTransaction(ctx, r.backend, tx)
	return hash, err
}
