...
import (
	...
	"github.com/bfe2021/go-bfe/accounts/external"
	"github.com/bfe2021/go-bfe/accounts/keystore"
	"github.com/bfe2021/go-bfe/accounts/scwallet"
	"github.com/bfe2021/go-bfe/accounts/usbwallet"
...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of gbfe is "gbfe". If no
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "Gbfe", keep that.
	if name == "gbfe" || name == "gbfe-testnet" {
		name = "Gbfe"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "gbfe" instances.
var isOldGbfeResource = map[string]bool{
	"chaindata":          true,
	"nodes":              true,
	"nodekey":            true,
...
func (c *Config) ResolvePath(path string) string {
	...
	// by gbfe 1.4 are used if they exist.
	if warn, isOld := isOldGbfeResource[path]; isOld {
		oldpath := ""
		if c.name() == "gbfe" {
		...
		if oldpath != "" && common.FileExist(oldpath) {
			if warn {
				c.warnOnce(&c.oldGbfeResourceWarning, "Using deprecated resource file %s, please move this file to the 'gbfe' subdirectory of datadir.", oldpath)
			}
			return oldpath
		}
	}
...
func makeAccountManager(conf *Config) (*accounts.Manager, string, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = ioutil.TempDir("", "go-bfe-keystore")
		ephemeral = keydir
	}
