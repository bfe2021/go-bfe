...
import (
	...
	"github.com/bfe2021/go-bfe/internal/testlog"
...
func TestRemoteNotify(t *testing.T) {
	...
	// Create the custom bfeash engine.
	bfeash := NewTester([]string{server.URL}, false)
	defer bfeash.Close()
	...
	bfeash.Seal(nil, block, nil, nil)
	select {
	case work := <-sink:
		if want := bfeash.SealHash(header).Hex(); work[0] != want {
...
func TestRemoteMultiNotify(t *testing.T) {
	...
	// Create the custom bfeash engine.
	bfeash := NewTester([]string{server.URL}, false)
	bfeash.config.Log = testlog.Logger(t, log.LvlWarn)
	defer bfeash.Close()
...
func TestStaleSubmission(t *testing.T) {
	...
	for id, c := range testcases {
		for _, h := range c.headers {
			bfeash.Seal(nil, types.NewBlockWithHeader(h), results, nil)
		}
		if res := api.SubmitWork(fakeNonce, bfeash.SealHash(c.headers[c.submitIndex]), fakeDigest); res != c.submitRes {
		...
		case <-time.NewTimer(time.Second).C:
			t.Errorf("case %d fetch bfeash result timeout", id+1)
		}
	}
}
