...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Gbfe.LightScryptN, Gbfe.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.bfedu", new NodeConfig());
			node.start();
			...
			BfeduClient ec = node.getBfeduClient();
	...
	// signatures too. Regression test for go-bfe issue #14599.
...
func TestAndroid(t *testing.T) {
	...
	workspace, err := ioutil.TempDir("", "gbfe-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/bfedu/gbfetest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for Gbfe and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.bfedu", "github.com/bfe2021/go-bfe/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	cp.CopyFile(filepath.Join("libs", "gbfe.aar"), "gbfe.aar")

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "bfedu", "gbfetest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
